(function(){var version="1.3.2";var author="Vova Feldman";var global=this;if(typeof(RW_Advanced_Options)==="undefined"){RW_Advanced_Options={};}
...
var doc=document,body=document.body,win=window,nav=navigator;var DUMMY_STR="DUMMY",UNDEF="undefined",EMPTY_PCID="00000000-0000-0000-0000-000000000000";var is_IE=false,is_FF=false;if(nav.userAgent){is_IE=nav.userAgent.match(/MSIE/)?true:false;is_FF=nav.userAgent.match(/Firefox/)?true:false;}
global.RW=function()
...
enrich_options_by_size(pOptions);coptions[pClass]=pOptions;},render:function(callback,optimized)
{optimized=this._getDefaultValue(optimized,true);render_options.optimized=optimized;render_options.callback=callback;var data=[];var lngs_data=[];var all_ratings=RW._getByClassName("rw-ui-container","div");if(optimized&&all_ratings.length==0)
...
rating.innerHTML='<img src="http://rating-widget.com/img/rw.loader.gif" alt="" />';var urid=getRatingId(rating);if(!urid){RW._error('Your rating widget container must include a rating id. E.g. <div class="rw-ui-container rw-urid-17"></div>');return;}
...
this._jsCall("http://rating-widget.com/js/action/load.php",data);},_isEqualImg:function(pImg1,pImg2)
...
{ratings_data[load_call_num]=res.data;load_call_num++;if(ratings_data.length>load_call_num)
{var data=[{name:"next",value:""}];data.push({name:"ratings",value:encodeURIComponent(RW.JSON.stringify(ratings_data[load_call_num]))});this._jsCall("http://rating-widget.com/js/action/load.php",data);return;}
res.data=[];for(var c=0,len_c=ratings_data.length;c<len_c;c++){res.data=res.data.concat(ratings_data[c]);}}
...
var css_load_data={};var inline_css_data={classes:{},themes:{}};if(this._is(res.data)&&this._isArray(res.data))
{for(var i=0,len=res.data.length;i<len;i++)
{if(!RW._is(res.data[i])){continue;}
var urid=res.data[i].urid;if(this._is(options[urid].rclass)&&!options[urid].rclass.isEmpty()){}else if(this._is(defaults.rclass)&&!defaults.rclass.isEmpty()){options[urid].rclass=defaults.rclass;}else{options[urid].rclass="";}
...
ratings[urid]=new RW.Rating(res.data[i],elements[urid],options[urid]);var type=options[urid].type,style=options[urid].style,size=options[urid].size;if(!this._is(css_load_data[type])){css_load_data[type]={};}
if(!this._is(css_load_data[type][size])){css_load_data[type][size]={};}
css_load_data[type][size][style]=true;}}
var href="http://rating-widget.com/css/external.php?"+
...
{var d=doc.createElement("div");d.className="rw-ui-container";body.appendChild(d);var computed_style=RW._getStyle(d,"display");d.parentNode.removeChild(d);if(computed_style!="inline-table"&&(!is_IE||computed_style!="block")&&(!is_FF||computed_style!="inline-block"))
{setTimeout(on_css_ready,100);}
else
{for(var urid in ratings){ratings[urid].render();}
if(RW._isCallable(callback)){callback(ratings);}
RW._clearJS();delete ratings_data;ratings_data=null;RW._invokeHook("RW_HOOK_READY");}};setTimeout(on_css_ready,100);},_rateCallback:function(res,urid)
...
return height;};this.options=pOptions;this.advanced=this.options.advanced;this.beforeRate=(RW._isCallable(this.options.beforeRate)?beforeRate:false);this.afterRate=(RW._isCallable(this.options.afterRate)?afterRate:false);this.rid=pData.rid;this.urid=pData.urid;this.uid=pData.uid;this.rate=parseFloat(pData.rate);this.votes=Math.max(0,parseInt(pData.votes,10));this.rate=Math.max(0,Math.min(RW.MAX*this.votes,this.rate));this.type=options.type;this.rated=pData.rated;this.voteRate=(this.rated)?pData.vote_rate:0;if(this.rated&&!(this.options.reVote)){this.options.readOnly=true;}
...
RW._foreach(labels,function(l){l.style.color=pColor;});font_color=(pColor===RW._defaultOptions.advanced.font.color)?"":"color: "+pColor+";";font.color=pColor;};this.setFontSize=function(pSize)
...
{var computedStyle,offsetParent=e.offsetParent,prevOffsetParent=e,doc=e.ownerDocument,docElem=doc.documentElement,body=doc.body,defaultView=doc.defaultView,prevComputedStyle=defaultView?defaultView.getComputedStyle(e,null):e.currentStyle,top=e.offsetTop,left=e.offsetLeft;while((e=e.parentNode)&&e!==body&&e!==docElem)
...
var place_tooltip=function(pTarget)
{if(rating.callState!="ready"){return;}
if(!rating.options.showTooltip){return;}
RW._tooltip.style.left="";RW._tooltip.style.top="";pTarget.appendChild(RW._tooltip);RW._tooltip.style.display="block";RW._tooltip.style.left="-"+Math.ceil((get_width(RW._tooltip)-icon_width)/2)+"px";var offset=get_offset(RW._tooltip),doc_offset=get_document_offset();body.appendChild(RW._tooltip);RW._tooltip.style.left=offset.left+"px";var height=get_height(RW._tooltip);if((offset.top-height-6)>doc_offset.top){RW._Class.set(RW._tooltip,"rw-ui-tooltip rw-valign-top");RW._tooltip.style.top=(offset.top-height-6)+"px";}else{RW._Class.set(RW._tooltip,"rw-ui-tooltip rw-valign-bottom");RW._tooltip.style.top=(offset.top+height-4)+"px";}};var render_nero=function()
...
rating.refreshCount=0;rating.refreshUI=false;clearTimeout(voteTimeout);voteTimeout=setTimeout(timer_refresh,wakeup_fragment);RW._jsCall("http://rating-widget.com/js/action/rate.php",data);};this.like=function(elm,like)
{if(rating.callState!="ready"){return;}
...
if(rating.options.uarid>0){data.push({name:"uarid",value:rating.options.uarid});}
rating.refreshCount=0;rating.refreshUI=false;clearTimeout(voteTimeout);voteTimeout=setTimeout(timer_refresh,wakeup_fragment);RW._jsCall("http://rating-widget.com/js/action/rate.php",data);};this.dislike=function(elm)
{return rating.like(elm,false);};this.setLabel=function(label)
{if(rating.callState!="ready"){return;}
if(!rating.options.showInfo){return;}
RW._foreach(labels,function(l){l.innerHTML=label;});};this.setTooltip=function(tip)
{if(rating.callState!="ready"){return;}
if(!rating.options.showTooltip){return;}

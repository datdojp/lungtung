...
{return!!(o&&undefined!=o&&o instanceof Object&&o.itxtClass);},isURL:function(s,i)
{if('string'!=typeof s)
{return false;}
...
{this.cacheImage(imgs[i]);}},serialiseJSON:function(object)
...
{var val=obj[elem];if(null!=val&&'function'!=typeof val&&'itxt'!=elem.substr(0,4)&&'$'!=elem.substr(0,1))
{var pass='';var name='';if(isNaN(elem))
{name+=elem+':';}
if($iTXT.core.Util.isArray(val))
{next=_iter(val,(depth+1));if(''!=next)
{pass+=name+'['+next+']';}}
...
{var delim=(val.match(/^(\{|\[\{).*(\}\]|\})$/))?'':'"';pass+=name+delim+val+delim;}
if(c>0&&''!=pass&&''!=out)
{pass=','+pass;}
c++;out+=pass;}}
if(0==depth)
...
{var fbDb=($iTXT.glob.dbgParams&&$iTXT.glob.dbgParams.get('firebugdb'))?$iTXT.glob.dbgParams.get('firebugdb'):-1;fbDb=('string'==typeof fbDb&&'all'==fbDb.toLowerCase())?logType.value:fbDb;if((fbDb==logType.value||('string'==typeof fbDb&&fbDb.toLowerCase()==logType.name.toLowerCase()))&&'string'==typeof logOpts)
...
{var undefined;$iTXT.core.Regex={hexColor:new RegExp("^([a-f]|[A-F]|[0-9]){3}(([a-f]|[A-F]|[0-9]){3})?$"),tagSrc:new RegExp("(SRC|src)=\"([^\"]*)\""),tagWidth:new RegExp("(WIDTH|width)=\"([^\"]*)\""),tagHeight:new RegExp("(HEIGHT|height)=\"([^\"]*)\""),stripTags:new RegExp("(<([^>]+)>)","gi"),stripNotImgBrScriptTags:new RegExp("(<(?!img|br|script|/script)([^>]+)>)","gi")};}
...
{if('string'==typeof children[i]||!isNaN(children[i]))
